...
if df_all.empty or "TIP" not in df_all["Ticker"].values:
    ...
    with c_a:
        ...
        # ------------------ [NEW] PDF 화일 데이터 완벽 시각화 연동 인터랙티브 리포트 ------------------
        with st.expander("📊 상세 역사 백테스트 리포트 [22.6 / -6.1 (Reinforced)_Final]", expanded=False):
            st.markdown("""
            **PDF 보고서 원본 기반 분석 데이터 (2017-11-01 ~ 2024-12-27)**
            본 데이터는 물가연동채(TIP) 카나리아 위험 회피 옵션과 강화된 모멘텀 로직을 실제 적용하여 장기 검증된 성과 리포트 내용입니다.
            """)
            ...
            with pdf_tab1:
                st.markdown("#### 🏆 성과 비교 요약 (vs SPY 벤치마크)")
                
                m_col1, m_col2, m_col3 = st.columns(3)
                m_col1.metric("포트폴리오 누적 수익률", "425.1%", "SPY 160.6%")
                m_col2.metric("연환산 수익률 (CAGR)", "25.7%", "SPY 14.1%")
                m_col3.metric("최대 낙폭 (MDD)", "-6.0%", "SPY -23.9%", delta_color="inverse")
                
                m_col4, m_col5, m_col6 = st.columns(3)
                m_col4.metric("샤프 지수 (Sharpe)", "1.84", "SPY 0.71")
                m_col5.metric("소티노 지수 (Sortino)", "5.09", "SPY 1.10")
                m_col6.metric("수익 월 비중 (Win Rate)", "72.1%", "62 / 86 개월")
                
                st.markdown("#### 📅 월별 수익률 성과 매트릭스 (%)")
                monthly_perf_data = {
                    "연도": ["2017", "2018", "2019", "2020", "2021", "2022", "2023", "2024"],
                    "1월": ["-", "5.4", "0.4", "-0.9", "2.4", "4.1", "-2.9", "3.5"],
                    "2월": ["-", "-1.3", "0.3", "-2.5", "-1.0", "1.7", "3.7", "7.5"],
                    "3월": ["-", "-2.3", "4.0", "10.0", "0.1", "6.7", "-3.1", "3.0"],
                    "4월": ["-", "1.2", "4.0", "3.2", "3.6", "2.7", "-0.3", "-4.7"],
                    "5월": ["-", "3.4", "-4.6", "3.9", "4.3", "-0.6", "7.2", "6.6"],
                    "6월": ["-", "1.7", "6.6", "9.0", "7.0", "0.8", "6.1", "4.7"],
                    "7월": ["-", "0.1", "1.6", "4.0", "2.2", "-3.2", "3.6", "-3.1"],
                    "8월": ["-", "4.1", "6.8", "-2.7", "1.0", "4.5", "-2.7", "0.4"],
                    "9월": ["-", "1.8", "-0.8", "0.0", "-1.7", "5.1", "4.3", "1.7"],
                    "10월": ["-", "0.6", "2.7", "15.7", "8.9", "1.7", "2.7", "0.1"],
                    "11월": ["-0.0", "-1.2", "1.3", "8.6", "-1.6", "-3.3", "-3.6", "1.9"],
                    "12월": ["0.1", "0.2", "3.3", "-0.5", "0.4", "2.0", "3.0", "1.4"],
                    "연간 성과": ["0.1%", "14.3%", "31.2%", "53.6%", "18.5%", "24.1%", "15.7%", "24.8%"]
                }
            ...
            with pdf_tab2:
                ...
                st.info("🦠 **코로나 팬데믹 폭락기 (2020/01 ~ 2020/03)**: S&P 500 지수가 급락하는 동안, 본 포트폴리오는 단 **-3.4%**의 낙폭만 허용하며 절대적인 방어력을 증명했습니다.")
                
                st.markdown("#### 📉 드로우다운(Drawdown) 손실 하락폭 순위 Top 10")
                drawdown_data = [
                    {"순위": 1, "하락 시작": "2020-01", "최대 하락": "2020-03", "회복 완료": "2020-04", "회복 기간": "1개월", "하락률 (MDD)": "-6.0%"},
                    {"순위": 2, "하락 시작": "2024-04", "최대 하락": "2024-04", "회복 완료": "2024-05", "회복 기간": "2개월", "하락률 (MDD)": "-4.7%"},
                    {"순위": 3, "하락 시작": "2019-05", "최대 하락": "2019-05", "회복 완료": "2019-06", "회복 기간": "1개월", "하락률 (MDD)": "-4.6%"},
                    {"순위": 4, "하락 시작": "2022-11", "최대 하락": "2023-01", "회복 완료": "2023-05", "회복 기간": "4개월", "하락률 (MDD)": "-4.1%"},
                    {"순위": 5, "하락 시작": "2023-11", "최대 하락": "2023-11", "회복 완료": "2024-01", "회복 기간": "3개월", "하락률 (MDD)": "-3.6%"},
                    {"순위": 6, "하락 시작": "2018-02", "최대 하락": "2018-03", "회복 완료": "2018-05", "회복 기간": "3개월", "하락률 (MDD)": "-3.6%"},
                    {"순위": 7, "하락 시작": "2022-07", "최대 하락": "2022-07", "회복 완료": "2022-08", "회복 기간": "2개월", "하락률 (MDD)": "-3.2%"},
                    {"순위": 8, "하락 시작": "2024-07", "최대 하락": "2024-07", "회복 완료": "2024-11", "회복 기간": "5개월", "하락률 (MDD)": "-3.1%"},
                    {"순위": 9, "하락 시작": "2023-08", "최대 하락": "2023-08", "회복 완료": "2023-09", "회복 기간": "1개월", "하락률 (MDD)": "-2.7%"},
                    {"순위": 10, "하락 시작": "2020-09", "최대 하락": "2020-09", "회복 완료": "2020-11", "회복 기간": "3개월", "하락률 (MDD)": "-2.7%"}
                ]
            ...
            with pdf_tab3:
                ...
                asset_perf_data = [
                    {"자산명 (ETF)": "QQQ (나스닥 100)", "CAGR (연성장률)": "19.6%", "변동성": "23.8%", "MDD": "-35.1%", "샤프 지수": "0.73", "포트폴리오 기여도": "16.4%"},
                    {"자산명 (ETF)": "SMH (글로벌 반도체)", "CAGR (연성장률)": "26.2%", "변동성": "33.7%", "MDD": "-45.3%", "샤프 지수": "0.71", "포트폴리오 기여도": "21.1%"},
                    {"자산명 (ETF)": "XLK (정보기술 Sector)", "CAGR (연성장률)": "21.6%", "변동성": "25.8%", "MDD": "-33.6%", "샤프 지수": "0.75", "포트폴리오 기여도": "10.9%"},
                    {"자산명 (ETF)": "GLD (골드 금현물)", "CAGR (연성장률)": "10.1%", "변동성": "14.2%", "MDD": "-22.0%", "샤프 지수": "0.56", "포트폴리오 기여도": "6.8%"},
                    {"자산명 (ETF)": "XLV (헬스케어 Sector)", "CAGR (연성장률)": "9.6%", "변동성": "17.4%", "MDD": "-28.4%", "샤프 지수": "0.43", "포트폴리오 기여도": "1.1%"},
                    {"자산명 (ETF)": "IBB (바이오테크)", "CAGR (연성장률)": "3.7%", "변동성": "23.6%", "MDD": "-39.8%", "샤프 지수": "0.07", "포트폴리오 기여도": "1.2%"},
                    {"자산명 (ETF)": "XLE (에너지 Sector)", "CAGR (연성장률)": "7.8%", "변동성": "32.3%", "MDD": "-66.8%", "샤프 지수": "0.18", "포트폴리오 기여도": "7.8%"},
                    {"자산명 (ETF)": "LIT (배터리 소재)", "CAGR (연성장률)": "2.5%", "변동성": "32.1%", "MDD": "-62.6%", "샤프 지수": "0.01", "포트폴리오 기여도": "12.9%"}
                ]
                st.dataframe(pd.DataFrame(asset_perf_data), use_container_width=True, hide_index=True)
                st.caption("* 데일리 기준 시뮬레이션 지표값")
