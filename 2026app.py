...
if df_all.empty or "TIP" not in df_all["Ticker"].values:
    ...
    with c_a:
        ...
        # ------------------ [NEW] 최신 PDF 보고서 데이터 완벽 시각화 연동 인터랙티브 리포트 (EEM & QTUM 강화본) ------------------
        with st.expander("📊 상세 역사 백테스트 리포트 [전략A+EEM+QTUM (31.5% / -6.7%)]", expanded=True):
            st.markdown("""
    ...
    with c_c:
        ...
        with st.expander("📖 전략 C 상세 운용원칙 및 기대성과 (섹터로테이션_전략_운용원칙_분석)"):
            ...
            * **2단계 (자산 매수 가이드)**:
                ...
                - **방어 신호 발생 시**: 원자재 방어 자산군(GLD, PDBC, OILK) 중 단순평겨 모멘텀 스코어가 가장 높은 1개 자산에 **$100\%$ 대피**하되, 모멘텀이 모두 마이너스이면 안전하게 **현금($100\%$)**을 확보합니다.
